...
try:
    ...
    print("Please install it using: pip install . (from the project root)", file=sys.stderr)
    sys.exit(1)
...
def convert_price_per_1m_to_1k(price_per_1m):
    ...
    if price_per_1m is None: return None
    try:
        # Remove '$' if present and convert to float
        price_str = str(price_per_1m).replace('$', '').strip()
        # Handle potential ranges like "1.50 - 3.00" - take the lower value for now
        if '-' in price_str: price_str = price_str.split('-')[0].strip()
        return float(price_str) / 1000.0
    except (ValueError, TypeError):
        print(f"-- Warning: Could not convert price '{price_per_1m}' to 1k unit.", file=sys.stderr)
        return None

def get_price_value(original_price_1m, unit):
    """Returns the price value based on the selected unit."""
    if unit == '1k':
        return convert_price_per_1m_to_1k(original_price_1m)
    else: # Default to 1m
        # Try to convert to float directly, handling '$' and potential errors
        if original_price_1m is None: return None
        try:
             price_str = str(original_price_1m).replace('$', '').strip()
             if '-' in price_str: price_str = price_str.split('-')[0].strip()
             return float(price_str)
        except (ValueError, TypeError):
             print(f"-- Warning: Could not convert price '{original_price_1m}' to 1m unit.", file=sys.stderr)
             return None

def model_to_dict(entry, unit):
    """Converts LLMModelPricing object to dict based on selected unit."""
    input_cost = get_price_value(entry.input_tokens_price, unit)
    output_cost = get_price_value(entry.output_tokens_price, unit)
    context_tokens = parse_context_window(entry.context)
    price_key_suffix = f"_cost_per_{unit}_tokens"
    return {
        "model": entry.model,
        f"input{price_key_suffix}": input_cost,
        f"output{price_key_suffix}": output_cost,
        "context_window_tokens": context_tokens,
        "source": entry.source,
        "updated": entry.updated
    }

def generate_lua_output(data, filename, source_description, unit):
    """Generates the Lua output file with prices in the specified unit."""
    print(f"-- Generating Lua output file: {filename} (Unit: {unit})", file=sys.stderr)
    price_unit_str = f"per {unit} tokens"
    price_key_suffix = f"_cost_per_{unit}_tokens"
    try:
        ...
        with open(filename, 'w', encoding='utf-8') as f_out:
            ...
            f_out.write(f"-- Prices are {price_unit_str}\n")
            ...
            for provider in provider_keys_sorted:
                ...
                for entry in models:
                    ...
                    if not model_key or model_key in model_entries: continue
                    ...
                    model_lua_parts.append(f'input{price_key_suffix} = {format_lua_value(input_cost)}')
                    model_lua_parts.append(f'output{price_key_suffix} = {format_lua_value(output_cost)}')
...
def generate_json_output(data, filename, source_description, unit):
    """Generates the JSON output file with prices in the specified unit."""
    print(f"-- Generating JSON output file: {filename} (Unit: {unit})", file=sys.stderr)
    price_unit_str = f"USD per {unit} tokens"
    output_structure = {
        "metadata": {
    ...
    try:
        ...
        for provider, models in provider_models.items():
            for entry in models:
                 model_dict = model_to_dict(entry, unit)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-u", "--unit",
                        choices=['1m', '1k'],
                        default='1m',
                        help="Price unit for output: per 1 Million tokens (1m) or per 1 Thousand tokens (1k) (default: 1m)")
    args = parser.parse_args()

    target_source_input = args.source.upper()
    output_format = args.format.lower()
    output_unit = args.unit.lower()
    ...
    if target_source_input == "ALL":
        ...
        for entry in all_pricing_data:
             # Prioritize entry if it doesn't exist or if new entry has a valid price and old one doesn't
             # (Simple merge logic, could be enhanced for freshness/priority)
            if entry.model not in combined_models_dict or \
               (combined_models_dict[entry.model].input_tokens_price is None and entry.input_tokens_price is not None):
        ...
        generate_output_func(merged_data, output_filename, "All enabled sources", output_unit)
        ...
        try:
            ...
            generate_output_func(pricing_data, output_filename, target_source_name, output_unit)
        except Exception as e:
